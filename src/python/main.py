...
def evaluate_operator(lhs, operator_label, rhs):
    """Compare Left-Hand-Side value with Right-Hand-Side value.
    Returns False when lhs or rhs can't be interpreted as numbers.
    """
    try:
        lhs_val = float(lhs)
        rhs_val = float(rhs)
...
def CleanCSV(df):
    ...
    # keep date column (not reformat to string). Many functions expect datetime dtype.
    return df


# Example main guard (adapt paths as needed)
if __name__ == "__main__":
    dag = json.load(open("../../main.json"))
    df = pd.read_csv(
        "/home/tushar/Downloads/MacroTrends_Data_Download_MSFT(1).csv")
    df = CleanCSV(df)
    equity, trades = run_backtest(
        df, dag, initial_capital=10000, allocation_fraction=0.10, commission=0.001)
